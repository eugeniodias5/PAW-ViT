...
def eval_impl(anno, res, partial=False):
    ...
    aps_sing, aps_multi = [], []
    cmc_sing, cmc_multi = np.zeros(len(eids), dtype=np.int32), np.zeros(
        len(eids), dtype=np.int32
    )
    for ans in res:
...
def exclude(eid, fid, eids, fids):
    ...
    eid_match = eids == eid
    cid_match = np.logical_and(fid[0] == fids[:, 0], fid[1] == fids[:, 1])
...
def evaluate_submission_atrw(anno, res, phase_codename, **kwargs):
    """
